...
//        return tp.underlineThickness;
    ...
    public static float getTextRunAdvances(Paint tp, char[] chars, int index, int count,
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P){//9.0 and up
                return tp.getTextRunAdvances(chars, index, count, contextIndex, contextCount, isRtl, advances, advancesIndex);
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) { // 5.0 above
    ...
    public static float getTextRunAdvances(Paint tp, CharSequence text, int start, int end,
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P){//9.0 and up
                ...
                float result = tp.getTextRunAdvances(buf, start - contextStart, len,
                        0, contextLen, isRtl, advances, advancesIndex);
                TemporaryBuffer.recycle(buf);
                return result;
                //String val = String.valueOf(text);
               // return tp.getTextRunAdvances(val.toCharArray(), start, end, contextStart, contextEnd, isRtl, advances, advancesIndex);
            }
            else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) { // 5.0 above
                return (float) MethodReflection.callAny(tp, "getTextRunAdvances",
