 ...
 */
public class EditorPreferences implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static final String KEY_WHOLE_WORDS_ONLY = "pref_whole_words_only";
    public static final String KEY_MATCH_CASE = "pref_match_case";
   ...
   // private static final int[] THEMES = new int[]{R.style.LightTheme, R.style.DarkTheme};
    ...
    public static EditorPreferences getInstance(Context context) {
        if (instance == null) {
            instance = new EditorPreferences(context.getApplicationContext());
        }
        return instance;
    }
    ...
    public boolean isHidePanel() {
        return (boolean) map.get(KEY_HIDE_SYMBOL_PANEL);
    }

    public void setHidePanel(boolean b) {
        preferences.edit().putBoolean(KEY_HIDE_SYMBOL_PANEL, b).apply();
        map.put(KEY_HIDE_SYMBOL_PANEL, b);
    }
    ...
    public boolean isOpenLastFiles() {
        return getBoolean(KEY_REMEMBER_LAST_OPENED_FILES, true);
    }
