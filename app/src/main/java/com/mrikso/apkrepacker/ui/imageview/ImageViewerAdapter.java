...
public class ImageViewerAdapter extends ViewPagerAdapter {
    ...
    public View onCreateView(@NonNull ViewGroup container, int position) {
        ...
        if (!FileUtil.FileType.getFileType(new File(currentPath)).equals(FileUtil.FileType.IMAGE) && new VectorMasterView(container.getContext(), new File(currentPath)).isVector()) {
            loadVector(holder, currentPath);
//            loadImage(holder, new VectorMasterDrawable(container.getContext(), new File(currentPath)));
        } else {
...
//        ViewUtils.fadeIn(holder.progress);
       ...
       // holder.vector.setUseLegacyParser(false);
        holder.vector.setVectorFile(new File(path));
//            VectorMasterView vectorMasterView = new VectorMasterView(holder.image.getContext(), new File(path));
    }
    ...
    public void onDestroyView(@NonNull ViewGroup container, int position, @NonNull View view) {
        ...
        Glide.with(container.getContext()).clear(holder.image);
