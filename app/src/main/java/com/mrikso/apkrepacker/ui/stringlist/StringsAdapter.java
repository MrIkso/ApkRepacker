...
public class StringsAdapter extends BaseAdapter implements Filterable {
    ...
    @Override
    public int getCount() {
        return dataFiltret.size();
    }

    @Override
    public Object getItem(int position) {
        return Iterables.get(dataFiltret.entrySet(), position);
    }
    ...
    public void setUpdateValue(String key, String value){
        if(dataFiltret.containsKey(key) && data.containsKey(key)){
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                dataFiltret.replace(key, value);
                data.replace(key, value);
            }
            else{
                dataFiltret.remove(key);
                dataFiltret.put(key, value);
                data.remove(key);
                data.put(key, value);
            }
        }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Map.Entry<String, String> item = Iterables.get(dataFiltret.entrySet(), position);
    ...
    public Filter getFilter() {
        Filter filter = new Filter() {
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                ...
                dataFiltret = (Map<String, String>) results.values;
                notifyDataSetChanged();
