...
final public class AndrolibResources {
    ...
    public ResPackage loadMainPkg(ResTable resTable, ExtFile apkFile)
            throws AndrolibException {
        LOGGER.info(R.string.log_text,"Loading resource table...");
        ...
        switch (pkgs.length) {
            ...
            case 2:
                if (pkgs[0].getName().equals("android")) {
                    LOGGER.warning(R.string.log_text,"Skipping \"android\" package group");
                    pkg = pkgs[1];
                    break;
                } else if (pkgs[0].getName().equals("com.htc")) {
                    LOGGER.warning(R.string.log_text,"Skipping \"htc\" package group");
                    pkg = pkgs[1];
                    break;
                }
    ...
    public ResPackage loadFrameworkPkg(ResTable resTable, int id, String frameTag)
        ...
        LOGGER.info(R.string.log_text,"Loading resource table from file: " + apk);
    ...
    public void decodeManifest(ResTable resTable, ExtFile apkFile, File outDir)
        ...
        try {
            ...
            LOGGER.info(R.string.log_text,"Decoding AndroidManifest.xml with only framework resources...");
    ...
    public void adjustPackageManifest(ResTable resTable, String filePath)
        ...
        if (pkgOriginal.equalsIgnoreCase(mPackageRenamed) || (Arrays.asList(IGNORED_PACKAGES).contains(pkgOriginal))) {
            LOGGER.info(R.string.log_text,"Regular manifest package...");
        } else {
            LOGGER.info(R.string.log_text,"Renamed manifest package found! Replacing " + mPackageRenamed + " with " + pkgOriginal);
            ResXmlPatcher.renameManifestPackage(new File(filePath), pkgOriginal, LOGGER);
        }
    }
    ...
    public void decodeManifestWithResources(ResTable resTable, ExtFile apkFile, File outDir)
        ...
        try {
            inApk = apkFile.getDirectory();
            out = new FileDirectory(outDir);
            LOGGER.info(R.string.log_text,"Decoding AndroidManifest.xml with resources...");
    ...
    public void decode(ResTable resTable, ExtFile apkFile, File outDir)
        ...
        for (ResPackage pkg : resTable.listMainPackages()) {
            ...
            LOGGER.info(R.string.log_text,"Decoding file-resources...");
            for (ResResource res : pkg.listFiles()) {
                fileDecoder.decode(res, in, out);
            }

            LOGGER.info(R.string.log_text,"Decoding values */* XMLs...");
    ...
    public void emptyFrameworkDirectory() throws AndrolibException {
        ...
        if (! apk.exists()) {
            LOGGER.warning(R.string.log_text,"Can't empty framework directory, no file found at: " + apk.getAbsolutePath());
        } else {
            try {
                if (apk.exists() && dir.listFiles().length > 1 && ! apkOptions.forceDeleteFramework) {
                    LOGGER.warning(R.string.log_text,"More than default framework detected. Please run command with `--force` parameter to wipe framework directory.");
                } else {
                    for (File file : dir.listFiles()) {
                        if (file.isFile() && file.getName().endsWith(".apk")) {
                            LOGGER.info(R.string.log_text,"Removing " + file.getName() + " framework file...");
                            file.delete();
                        }
                    }
                }
    ...
    public void installFramework(File frameFile, String tag)
        ...
        try {
            ...
            zip.close();
            LOGGER.info(R.string.log_text,"Framework installed to: " + outFile);
        } catch (IOException ex) {
            throw new AndrolibException(ex);
        } finally {
