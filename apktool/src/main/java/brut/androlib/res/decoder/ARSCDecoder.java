...
public class ARSCDecoder {
    ...
    private ResPackage readTablePackage() throws IOException, AndrolibException {
        ...
        if (mTypeIdOffset > 0) {
            LOGGER.warning(R.string.log_report_error);
        }
    ...
    private void readLibraryType() throws AndrolibException, IOException {
        ...
        for (int i = 0; i < libraryCount; i++) {
            ...
            LOGGER.info(R.string.log_decoding_shared_library, packageName, packageId);
        }
    ...
    private void readTableTypeSpec() throws AndrolibException, IOException {
        ...
        while (type == Header.TYPE_TYPE) {
            ...
            if (mCountIn.getCount() < mHeader.endPosition) {
                LOGGER.warning(R.string.log_unknown_skip_bytes, mHeader.endPosition - mCountIn.getCount());
    ...
    private ResType readTableType() throws IOException, AndrolibException {
        ...
        if (position != mCountIn.getCount()) {
            LOGGER.warning(R.string.log_text,"Invalid data detected. Skipping: " + (position - mCountIn.getCount()) + " byte(s)");
            mIn.skipBytes(position - mCountIn.getCount());
        }

        if (typeFlags == 1) {
            LOGGER.info(R.string.log_text,"Sparse type flags detected: " + mTypeSpec.getName());
        }
        ...
        if (flags.isInvalid) {
            ...
            if (mKeepBroken) {
                LOGGER.warning(R.string.log_text,"Invalid config flags detected: " + resName);
            } else {
                LOGGER.warning(R.string.log_text,"Invalid config flags detected. Dropping resources: " + resName);
            }
        }
    ...
    private void readEntry(EntryData entryData) throws AndrolibException {
        ...
        try {
            ...
            if (mKeepBroken) {
                ...
                LOGGER.warning(R.string.log_text,String.format("Duplicate Resource Detected. Ignoring duplicate: %s", res.toString()));
            } else {
                throw ex;
            }
        }
    ...
    private ResConfigFlags readConfigFlags() throws IOException, AndrolibException {
        ...
        if (exceedingSize > 0) {
            ...
            if (exceedingBI.equals(BigInteger.ZERO)) {
                LOGGER.fine(R.string.log_text, String
                        .format("Config flags size > %d, but exceeding bytes are all zero, so it should be ok.",
                                KNOWN_CONFIG_BYTES));
            } else {
                LOGGER.warning(R.string.log_text,String.format("Config flags size > %d. Size = %d. Exceeding bytes: 0x%X.",
    ...
    public static class ARSCData {
        ...
        public ResPackage getOnePackage() throws AndrolibException {
            if (mPackages.length <= 0) {
                ...
                LOGGER.info(R.string.log_text,"Arsc file contains multiple packages. Using package "
