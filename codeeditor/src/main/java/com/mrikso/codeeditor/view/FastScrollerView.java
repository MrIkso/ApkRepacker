...
public class FastScrollerView extends View implements OnScrollChangedListener {
    ...
    public void link(FreeScrollingTextField editor) {
        if(editor != null) {
            mEditor = editor;
    ...
    private void scrollCodeView() {
        ...
        mEditor.scrollTo(mEditor.getScrollX(), ((int) (mScrollMax * scrollToAsFraction)) - ((int) (scrollToAsFraction * (mEditor.getHeight() - mEditor.mCaretRow))));
    }

    private int getThumbTop() {
        int absoluteThumbTop = Math.round((mViewHeight - mThumbHeight) * (mScrollY / ((mScrollMax - mEditor.getHeight()) + mEditor.mCaretRow)));
