...
public class MDRootLayout extends ViewGroup {
    ...
    private static boolean isVisible(View v) {
        ...
        if (visible && v instanceof Button) {
            visible = ((Button) v).getText().toString().trim().length() > 0;
        }
        return visible;
    }
    ...
    public void onFinishInflate() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (v.getId() == R.id.md_titleFrame) {
                ...
                buttons[INDEX_NEUTRAL] = (Button) v;
            } else if (v.getId() == R.id.md_buttonDefaultNegative) {
                buttons[INDEX_NEGATIVE] = (Button) v;
            } else if (v.getId() == R.id.md_buttonDefaultPositive) {
                buttons[INDEX_POSITIVE] = (Button) v;
            } else {
                content = v;
            }
        }
